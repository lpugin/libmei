...
MeiAttribute* mei::IncipCode::getXml() {
    if (!hasAttribute("xml")) {
        throw AttributeNotFoundException("xml");
    }
    return getAttribute("xml");
};

void mei::IncipCode::setXml(std::string _xml) {
    MeiAttribute *a = new MeiAttribute("xml", _xml);
    addAttribute(a);
};

bool mei::IncipCode::hasXml() {
    return hasAttribute("xml");
};

void mei::IncipCode::removeXml() {
    removeAttribute("xml");
};
